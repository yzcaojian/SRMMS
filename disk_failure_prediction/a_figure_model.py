...
@Description: 从backblaze数据集采集各类硬盘型号对应的故障记录数
@Time : 2021/1/31 12:05
@Author : cao jian
"""
...
def read_csv(file):
    ...
    for record in reader:
        ...
        if record[4] == '1':
            flag = True
            for item in fail_model.keys():
                if record[2] == item:
                    fail_model[item] = fail_model[item] + 1  # 该硬盘型号记录数量+1
                    flag = False
            if flag:
...
# # 生成20年:0101-1231的日期序列
# date = [datetime.strftime(x, '%Y-%m-%d') for x in list(pd.date_range('20200101', '20201231'))]
# for day in date:
#     filename = "D:\\backblaze hdd dataset\\2020\\" + day + ".csv"
#     read_csv(filename)
