...
def trainGRU(initial_learning_rate):
    ...
    with tf.compat.v1.Session(config=config) as sess:
        ...
        for epoch in range(epochs):
            ...
            test_accuracy = sess.run(accuracy, feed_dict={x: test_data[300:300+batch_size, :, :],
                                                          y: test_label[300:300+batch_size, :]})
            print("test_accuracy", test_accuracy)
            if test_accuracy > 0.82 and acc > 0.99:
                print("测试集精度超过82%，提前终止")
                true_epochs = epoch + 1
                break
        print("Optimization Finished!")
        print("model_save: ", saver.save(sess, './model_2/hdd_GRU_2_model.ckpt'))
        # model_2:  test accuracy:0.831    参数： lr=0.001，每1000步衰减5%，batch_size:256，epoch:1000, muilGRU
