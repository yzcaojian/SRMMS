...
@Time : 2021/12/3 16:36
@Author : cao jian
"""

file = open("TOSHIBA MQ01ABF050_v1.csv")
...
while t > 0:
    if seqlen[t] - seqlen[t - 1] > 30:
        ...
        while s >= t:
            ...
            s -= 1
    t = t - 1
print("经过筛选，将三十天以外的数据全部删除", hdd.shape)  # (6842, 25)
...
np.save("TOSHIBA MQ01ABF050_data_2_v1.npy", smart)
np.save("TOSHIBA MQ01ABF050_label_2_v1.npy", health_state)
np.save("seqlen_2.npy", seqlen)

smart = np.load("TOSHIBA MQ01ABF050_data_2_v1.npy")
health_state = np.load("TOSHIBA MQ01ABF050_label_2_v1.npy")
print(health_state[0], health_state[1], health_state[2])
print(smart[2])  # 打印测试程序是否正确
print(smart.shape)  # (6842, 24)
