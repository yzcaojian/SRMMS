...
def trainGRU(initial_learning_rate):
    ...
    with tf.compat.v1.Session(config=config) as sess:
        ...
        for epoch in range(epochs):
            ...
            test_accuracy = sess.run(accuracy, feed_dict={x: test_data[0:batch_size, :, :], y: test_label[0:batch_size, :]})
            print("test_accuracy", test_accuracy)
            if test_accuracy > 0.9 and acc > 0.99:
                print("测试集精度超过90%，提前终止")
                true_epochs = epoch + 1
                break
            if epoch % 10 == 0:
                shuffle_in_unison_scary(train_data, train_label)  # 每个epoch都对数据集重新打乱一次
        print("Optimization Finished!")
        print("model_save: ", saver.save(sess, './model/model/hdd_GRU_model.ckpt'))
        # model_learning:  test accuracy:0.824   参数： lr=0.001, 每1000步衰减5%，batch_size:256,epoch:2000, muilGRU
