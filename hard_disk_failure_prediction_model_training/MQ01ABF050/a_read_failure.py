...
title = ['serial_number', 'failure', 'smart_1_raw', 'smart_2_raw', 'smart_3_raw', 'smart_4_raw', 'smart_5_raw',
         'smart_7_raw', 'smart_8_raw', 'smart_9_raw', 'smart_10_raw', 'smart_12_raw', 'smart_191_raw', 'smart_192_raw',
         'smart_193_raw', 'smart_194_raw', 'smart_196_raw', 'smart_197_raw', 'smart_198_raw', 'smart_199_raw',
         'smart_220_raw', 'smart_222_raw', 'smart_223_raw', 'smart_224_raw', 'smart_226_raw', 'smart_240_raw']
...
for day in date:
    ...
    for record in reader:
        # 选择第一行列名部分的字段作为结果表列名
        # 生成新表属性列名，步骤三
        # if first_time and record[2] == "TOSHIBA MQ01ABF050":
        ...
        if record[2] == "TOSHIBA MQ01ABF050":  # 选择只采集东芝TOSHIBA MQ01ABF050型号的硬盘数据
            temp = []  # 临时保留有效的SMART数据
            # 读故障记录
            if record[4] == "1":
...
writer = csv.writer(open('TOSHIBA MQ01ABF050_v1.csv', 'a+', newline=''))  # 参数newline是用来控制文本模式之下一行的结束字符
