...
@Time : 2021/12/3 16:41
@Author : cao jian
"""

# 判断天数，不同天数划分不同的步长
smart = np.load('TOSHIBA MQ01ABF050_data_v2.npy')
...
def stepSlice(aDisk, sliceLen=20,  step=1):
    ...
    # 分段切片，每个区间取步长1:2:4:8:12:32，一共6个区间，每个区间保证10个输入RNN的序列
...
hdd = np.array(hdd)
print('所有的样本集', hdd.shape)  # (14417, 20, 10)
smart = hdd[:, :, 0:-1]
label = hdd[:, :, -1]
label = label.astype(np.int32)
print(smart.shape)
print('one-hot编码之前标签数组维度为', label.shape)  # (14417, 20)
...
print('one-hot编码之后标签数组维度为', label.shape)  # (14417, 6)
