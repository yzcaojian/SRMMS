...
@Time : 2021/12/3 16:39
@Author : cao jian
"""

reader = read_csv('TOSHIBA MQ01ABF050_v1.csv', header=0)
title = np.array(reader.columns)[2:]  # 取表头
smart = np.load('TOSHIBA MQ01ABF050_data_v1.npy')
label = np.load('TOSHIBA MQ01ABF050_label_v1.npy')
feature = smart[::2, :]  # 以1/2数据量作为特征选择
label = label[::2, ]
# smart = np.load('feature_selection_data_2.npy')
# label = np.load('feature_selection_label_2.npy')
# true_smart = np.load('TOSHIBA MQ01ABF050_data_2_v1.npy')
# feature = smart

print(feature.shape)  # (85952, 24) 一级  (2531, 24) 二级
print(label.shape)
...
for (support, feature, i) in zip(rfe.support_, title, range(len(title))):
    if support:
        ...
        print(feature, end=' ')
# 一级健康度
# smart_3_raw smart_9_raw smart_12_raw smart_191_raw smart_193_raw smart_194_raw smart_197_raw smart_222_raw smart_226_raw
# 二级健康度
# smart_3_raw smart_4_raw smart_9_raw smart_191_raw smart_193_raw smart_194_raw smart_197_raw smart_222_raw smart_226_raw

for i in range(len(title)):
    if i not in left_col:
        del_col.append(i)
smart = np.delete(smart, del_col, axis=1)
print('\n', smart.shape)  # (171904, 9)
np.save('TOSHIBA MQ01ABF050_data_v2.npy', smart)
...
# print('\n', true_smart.shape)
# np.save('TOSHIBA MQ01ABF050_data_2_v2.npy', true_smart)
