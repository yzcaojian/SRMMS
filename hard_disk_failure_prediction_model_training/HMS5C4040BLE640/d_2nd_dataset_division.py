...
# 判断天数，不同天数划分不同的步长
smart = np.load('HGST HMS5C4040BLE640_data_2_v2.npy')
label = np.load('HGST HMS5C4040BLE640_label_2_v1.npy')
...
hdd = np.array(hdd)
print('所有的样本集', hdd.shape)  # (4107, 20, 10)

smart = hdd[:, :, 0:-1]
label = hdd[:, :, -1]
label = label.astype(np.int32)
print('one-hot编码之前标签数组维度为', label.shape)  # (4107, 20)
...
print('one-hot编码之后标签数组维度为', label.shape)  # (4107, 3)
