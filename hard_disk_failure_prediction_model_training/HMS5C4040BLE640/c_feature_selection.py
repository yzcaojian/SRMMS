...
@Time : 2021/12/2 20:04
@Author : cao jian
"""

reader = read_csv('HGST HMS5C4040BLE640_v1.csv', header=0)
title = np.array(reader.columns)[2:]  # 取表头
smart = np.load('HGST HMS5C4040BLE640_data_v1.npy')
...
# true_smart = np.load('HGST HMS5C4040BLE640_data_2_v1.npy')

feature = smart  # 以全部的数据量作为特征
print(feature.shape)  # 一级 (150881, 17) 二级
print(label.shape)
...
for (support, feature, i) in zip(rfe.support_, title, range(len(title))):
    if support:
        ...
        print(feature, end=' ')
# 一级健康度
# smart_2_raw smart_3_raw smart_4_raw smart_9_raw smart_12_raw smart_192_raw smart_193_raw smart_194_raw smart_197_raw
# 二级健康度
# smart_1_raw smart_4_raw smart_5_raw smart_9_raw smart_10_raw smart_192_raw smart_193_raw smart_194_raw smart_197_raw

for i in range(len(title)):
    if i not in left_col:
        del_col.append(i)
smart = np.delete(smart, del_col, axis=1)
print('\n', smart.shape)  # (150881, 9)
np.save('HGST HMS5C4040BLE640_data_v2.npy', smart)

# for i in range(len(title)):
#     if i not in left_col:
#         del_col.append(i)
# true_smart = np.delete(true_smart, del_col, axis=1)
# print('\n', true_smart.shape)  # (7366, 9)
# np.save('HGST HMS5C4040BLE640_data_2_v2.npy', true_smart)
