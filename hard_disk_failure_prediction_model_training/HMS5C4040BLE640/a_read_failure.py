...
for day in date:
    ...
    for record in reader:
        # 选择第一行列名部分的字段作为结果表列名
        # 生成新表属性列名，步骤三
        # if first_time and record[2] == "HGST HMS5C4040BLE640":
        ...
        if record[2] == "HGST HMS5C4040BLE640":  # 选择只采集希捷HGST HMS5C4040BLE640型号的硬盘数据
            temp = []  # 临时保留有效的SMART数据
            # 读故障记录
            if record[4] == "1":
...
writer = csv.writer(open('HGST HMS5C4040BLE640_v1.csv', 'a+', newline=''))  # 参数newline是用来控制文本模式之下一行的结束字符
