...
# 判断天数，不同天数划分不同的步长
smart = np.load('ST4000DM000_data_v2.npy')
...
def stepSlice(aDisk, sliceLen=20,  step=1):
    ...
    # 分段切片，每个区间取步长1:2:4:8:16:32，一共6个区间，每个区间保证10个输入RNN的序列
    ...
    while start < border:
        if flag % 10 == 0 and flag != 0 and step <= 16:
            step = 2 * step
...
hdd = np.array(hdd)
print('所有的样本集', hdd.shape)
smart = hdd[:, :, 0:-1]
label = hdd[:, :, -1]
label = label.astype(np.int32)
print(smart.shape)
print('one-hot编码之前标签数组维度为', label.shape)  # (187954, 20)
...
print('one-hot编码之后标签数组维度为', label.shape)  # (187954, 6)
