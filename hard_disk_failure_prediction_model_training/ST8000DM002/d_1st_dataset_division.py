...
@Time : 2021/12/3 14:12
@Author : cao jian
"""

# 判断天数，不同天数划分不同的步长
smart = np.load('ST8000DM002_data_v2.npy')
...
def stepSlice(aDisk, sliceLen=20,  step=1):
    ...
    # 分段切片，每个区间取步长1:2:4:8:12:40，一共6个区间，每个区间保证10个输入RNN的序列
    # 针对此model的记录数量，平均每个盘为3360左右记录，可以使用如上的区间步长
    sliceList = []
...
hdd = np.array(hdd)
print('所有的样本集', hdd.shape)  # (25847, 20, 10)
smart = hdd[:, :, 0:-1]
label = hdd[:, :, -1]
label = label.astype(np.int32)
print(smart.shape)
print('one-hot编码之前标签数组维度为', label.shape)  # (25847, 20)
...
print('one-hot编码之后标签数组维度为', label.shape)  # (25847, 6)
