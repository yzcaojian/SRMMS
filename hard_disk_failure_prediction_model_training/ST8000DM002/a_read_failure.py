...
for day in date:
    ...
    for record in reader:
        # 选择第一行列名部分的字段作为结果表列名
        # 生成新表属性列名，步骤三
        # if first_time and record[2] == "ST8000DM002":
        ...
        if record[2] == "ST8000DM002":  # 选择只采集希捷ST12000NM0007型号的硬盘数据
            temp = []  # 临时保留有效的SMART数据
            # 读故障记录
            if record[4] == "1":
...
writer = csv.writer(open('ST8000DM002_v1.csv', 'a+', newline=''))  # 参数newline是用来控制文本模式之下一行的结束字符
