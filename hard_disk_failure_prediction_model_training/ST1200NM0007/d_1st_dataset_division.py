...
# 判断天数，不同天数划分不同的步长
smart = np.load('ST12000NM0007_data_v2.npy')
...
def stepSlice(aDisk, sliceLen=20,  step=1):
    ...
    # 分段切片，每个区间取步长1:2:4:8:16:16，一共6个区间，每个区间保证10个输入RNN的序列
    ...
    while start < border:
        if flag % 10 == 0 and flag != 0 and step <= 8:
            step = 2 * step
...
print(smart.shape)
print('one-hot编码之前标签数组维度为', label.shape)  # (102404, 20)
...
print('one-hot编码之后标签数组维度为', label.shape)  # (102404, 6)
