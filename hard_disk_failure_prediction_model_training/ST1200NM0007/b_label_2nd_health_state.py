...
print('seqlen:', seqlen.shape)  # (1805,)
...
while t > 0:
    if seqlen[t] - seqlen[t - 1] > 30:
        ...
        while s >= t:
            ...
            s -= 1
    t = t - 1
print("经过筛选，将三十天以外的数据全部删除", hdd.shape)  # (52581, 23)
...
np.save("ST12000NM0007_data_2_v1.npy", smart)
np.save("ST12000NM0007_label_2_v1.npy", health_state)
np.save("seqlen_2.npy", seqlen)
# writer = csv.writer(file2, 'a+', newline='')
# for record in hdd:
#     writer.writerow(record[1:])  # 无需保留序列号

smart = np.load("ST12000NM0007_data_2_v1.npy")
health_state = np.load("ST12000NM0007_label_2_v1.npy")
print(health_state[0], health_state[1], health_state[2])
print(smart[2])  # 打印测试程序是否正确
# print(hdd)
print(smart.shape)  # (52581, 22)
