...
print(train_data.shape)  # (92163, 20, 9)
...
def trainGRU(initial_learning_rate):
    ...
    learning_rate = tf.compat.v1.train.exponential_decay(initial_learning_rate, global_step=global_step,
                                                         decay_steps=6000, decay_rate=0.95)
    ...
    with tf.compat.v1.Session(config=config) as sess:
        ...
        for epoch in range(epochs):
            ...
            if test_accuracy > 0.9 and acc > 0.99:
                ...
                break
            if epoch % 20 == 0:
                shuffle_in_unison_scary(train_data, train_label)  # 每20个epoch都对数据集重新打乱一次
        print("Optimization Finished!")
        print("model_save: ", saver.save(sess, './model/hdd_GRU_model.ckpt'))
        # model_learning:  test accuracy:0.761   参数： lr=0.001, 每10000步衰减5%，batch_size:300, epoch:1200, muilGRU
