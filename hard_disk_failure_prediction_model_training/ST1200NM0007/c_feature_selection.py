...
@Time : 2021/12/2 13:55
@Author : cao jian
"""

reader = read_csv('ST12000NM0007_v1.csv', header=0)
title = np.array(reader.columns)[2:]  # 取表头
smart = np.load('ST12000NM0007_data_v1.npy')
label = np.load('ST12000NM0007_label_v1.npy')
feature = smart[::4, :]  # 以1/4数据量作为特征选择
label = label[::4, ]
# smart = np.load('feature_selection_data_2.npy')
# label = np.load('feature_selection_label_2.npy')
# true_smart = np.load('ST12000NM0007_data_2_v1.npy')
# feature = smart

print(feature.shape)  # (209110, 22) 一级  (19553, 22) 二级
print(label.shape)
...
for (support, feature, i) in zip(rfe.support_, title, range(len(title))):
    if support:
        ...
        print(feature, end=' ')
# 一级健康度
# smart_4_raw smart_5_raw smart_9_raw smart_192_raw smart_193_raw smart_197_raw smart_240_raw smart_241_raw smart_242_raw
# 二级健康度
# smart_5_raw smart_7_raw smart_9_raw smart_187_raw smart_192_raw smart_193_raw smart_240_raw smart_241_raw smart_242_raw
...
print('\n', smart.shape)
np.save('ST12000NM0007_data_v2.npy', smart)
...
# print('\n', true_smart.shape)
# np.save('ST12000NM0007_data_2_v2.npy', true_smart)
