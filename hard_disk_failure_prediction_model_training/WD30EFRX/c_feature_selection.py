...
@Time : 2021/5/24 10:16
@Author : cao jian
"""

reader = read_csv('WD30EFRX_v1.csv', header=0)
title = np.array(reader.columns)[2:]  # 取表头
# smart = np.load('WD30EFRX_data_v1.npy')
# label = np.load('WD30EFRX_label_v1.npy')
smart = np.load('feature_selection_data_2.npy')
label = np.load('feature_selection_label_2.npy')
true_smart = np.load('WD30EFRX_data_2_v1.npy')
print(smart.shape)  # (58823, 17)
feature = smart[::, :]  # 以全部数据量作为特征
label = label[::]
print(feature.shape)  # (58823, 17)
print(label.shape)  # (58823, )
...
for (support, feature, i) in zip(rfe.support_, title, range(len(title))):
    if support:
        ...
        print(feature, end=' ')
# 一级健康度
# smart_1_raw smart_3_raw smart_4_raw smart_9_raw smart_12_raw smart_192_raw smart_193_raw smart_194_raw smart_197_raw
# 二级健康度
# smart_1_raw smart_3_raw smart_4_raw smart_9_raw smart_12_raw smart_192_raw smart_193_raw smart_194_raw smart_197_raw
...
# print('\n', smart.shape)
# np.save('WD30EFRX_data_v2.npy', smart)

for i in range(len(title)):
    if i not in left_col:
        del_col.append(i)
true_smart = np.delete(true_smart, del_col, axis=1)
print('\n', true_smart.shape)  #  (4558, 9)
np.save('WD30EFRX_data_2_v2.npy', true_smart)
