...
def trainGRU(initial_learning_rate):
    ...
    learning_rate = tf.compat.v1.train.exponential_decay(initial_learning_rate, global_step=global_step,
                                                         decay_steps=250, decay_rate=0.9)
    ...
    with tf.compat.v1.Session(config=config) as sess:
        ...
        for epoch in range(epochs):
            ...
            test_acc = sess.run(accuracy, feed_dict={x: test_data, y: test_label})  # 每步看测试准确度
            print("test accuracy", test_acc)
        print("Optimization Finished!")
        print("model_save: ", saver.save(sess, './model_2_2/hdd_GRU_2_model.ckpt'))
        # model_2:  test accuracy:0.742    参数： lr=0.001，每250步衰减10%，batch_size:128，epoch:1000, muilGRU
        # model_2_2:  test accuracy:0.838    参数： lr=0.001，每250步衰减10%，batch_size:256，epoch:1000, muilGRU
