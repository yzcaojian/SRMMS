...
for i, record in enumerate(reader):
    ...
    # if first_time:
    #     hdd = np.array([[int(float(record[i])) for i in range(1, len(record))]])
    #     print(hdd.shape)
    #     first_time = False
    # temp = np.array([[int(float(record[i])) for i in range(1, len(record))]])
    # print(temp.shape)
    # np.row_stack((hdd, temp))
hdd = np.array(hdd)
...
for i in range(1, len(hdd)):
    if hdd[i, 0] == 1:  # failure字段是1，hdd[0, 0]一定是1
        ...
        # else:
        #     for k in (j + 730, i):  # 只保存两年内的数据，太久远的正常数据不需要
        #         del_col.append(k)
        #     for k in range(j, j + 730):  # 按距离故障天数划分一级健康度
        #         days = k - j
        #         if days >= 310:
        #             hdd[k, 0] = 6
        #         elif days >= 150:
...
print('序号数量', seqlen.shape)  # (3256, )，对应硬盘序列号数量
# for i in del_col:
#     print(hdd[i])
...
np.save("ST4000DM000_data_v1.npy", smart)
np.save("ST4000DM000_label_v1.npy", health_state)
...
health_state = np.load("ST4000DM000_label_v1.npy")
print('测试------------------------')
