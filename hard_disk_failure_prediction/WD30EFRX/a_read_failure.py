...
@Description: 以WD30EFRX型号硬盘读故障记录和故障前正常记录
@Time : 2021/5/24 10:06
@Author : cao jian
"""
...
title = ['serial_number', 'failure', 'smart_1_raw', 'smart_3_raw', 'smart_4_raw', 'smart_5_raw', 'smart_7_raw',
         'smart_9_raw', 'smart_10_raw', 'smart_11_raw', 'smart_12_raw', 'smart_192_raw', 'smart_193_raw',
         'smart_194_raw', 'smart_196_raw', 'smart_197_raw', 'smart_198_raw', 'smart_199_raw', 'smart_200_raw']
...
for day in date:
    ...
    for record in reader:
        # 选择第一行列名部分的字段作为结果表列名
        # 生成新表属性列名，步骤三
        # if first_time and record[2] == "WDC WD30EFRX":
        ...
        if record[2] == "WDC WD30EFRX":  # 选择只采集昱科HGST HMS5C4040ALE640型号的硬盘数据
            temp = []  # 临时保留有效的SMART数据
            # 读故障记录
            if record[4] == "1":
...
writer = csv.writer(open('WD30EFRX_v1.csv', 'a+', newline=''))
# writer.writerow(title[:len(template) + 1])  # 写入title，步骤五
writer.writerow(title)

print("样本数量", len(fail_list))
