...
@Description: 划分二级健康度
@Time : 2021/5/24 10:24
@Author : cao jian
"""

# 判断天数，不同天数划分不同的步长
smart = np.load('WD30EFRX_data_2_v2.npy')
label = np.load('WD30EFRX_label_2_v1.npy')
seq_len = np.load('seqlen_2.npy')

print('smart数据格式', smart.shape)
print('label数据格式', label.shape)
hdd = np.column_stack((smart, label))
print('hdd数据格式', hdd.shape)  # (4558, 10)
...
hdd = np.array(hdd)
print('所有的样本集', hdd.shape)  # (2499, 20, 10)

smart = hdd[:, :, 0:-1]
label = hdd[:, :, -1]
label = label.astype(np.int32)
print('one-hot编码之前标签数组维度为', label.shape)  # (2499, 20)
...
print('one-hot编码之后标签数组维度为', label.shape)  # (2499, 3)
...
train_data = smart[:1342, :, :]
train_label = label[:1342, :]
test_data = smart[1342:, :, :]  # 直接取128个样本数据作为测试集
test_label = label[1342:, :]
# partition = []
# i = 0
